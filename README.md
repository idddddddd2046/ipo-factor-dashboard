...
并展示现行估值模块的 PE/PS 组合、市场可比性降权、30 只历史回放覆盖与正式弃权状态；
不把相关性写成因果或个股建议。
...
| `valuation_status.json` | 现行估值模块规则、30 只历史回放、同业市场可比性与正式弃权的**只读快照** | 数据层：由 ipo-tool 估值回放报告导出 |
| `template.html` | 页面**模板**：CSS（`<style>`）+ 交互逻辑（`<script>`）+ 结构。含五份只读数据占位符 | 渲染层：直接改这里 |
| `build.py` | 校验五份快照的日期/计数契约，再塞进 template → `index.html`。纯 Python stdlib | 数据契约变化时才改 |
...
- **不改数据口径**：五份 JSON 都是 ipo-tool 冻结报告的快照，渲染层不得篡改统计数值；要改口径去数据层。
...
- **估值不是目标价**：跨市场同业只按 35%–60% 采用估值偏离；少于 3 家合格同业时正式弃权。30 只回放属于同样本方向性检查，不得宣传成独立验证。
