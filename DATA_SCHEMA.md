...
专家规则 D1/D30 审计级历史仿真、关系/锁定证据 dry-run 计划及四批共 69 只招股书逐页审阅导出。
...
- `long_factor_pool.production_change=false`：通过长期联合筛选只代表长期候选，不得写成已入生产。
- `display_factors[]` 同时给出 D180、D180 首日后超额、D365、D365 首日后超额及总覆盖率；网页不得只挑有利口径。
...
- `evidence_backfill.collection`：69 只目标样本的官方材料与人工裁决覆盖。`windowed=69`、`nonempty_page_evidence=69`、`manual_reviews_completed=69`、`queued_for_manual_review=0`；自动生成因子值恒为 0。
- `evidence_backfill.review.summary`：合并四批 69/69 只逐页结果，共 161 个可审计研究值；这是“值”的数量，不是 161 个有效因子。分项为严格关连人士 50 只、上市前股东 16 只、客户/供应商 6 只、基石锁定/上市公众持股 20 只、老股出售 69 只；锁定期组件另有 57 只。
- 完整审阅后，`cs_connected_person_share` 的 D180 归一 IC 为 −0.008（n=50，p=0.953），`cs_pre_ipo_shareholder_share` 为 −0.085（n=16，p=0.740），`secondary_sell_down_pct` 为 +0.018（n=69，p=0.925）；均未显示可靠长期预测力。客户/供应商仅 n=6，公众持股精确分母仅 n=20，继续 `collect_more_data`。
