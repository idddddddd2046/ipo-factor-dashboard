# -*- coding: utf-8 -*-
"""构建看板：dashboard_data.json + template.html -> index.html（纯 stdlib，无外部依赖）。

用法：
    python build.py

原理：template.html 里有一处占位符 `__DATA__`（在 <script> 的 `const DATA = __DATA__;`），
本脚本把 dashboard_data.json 原样塞进去，再包上 <!doctype> 骨架，输出 index.html。
